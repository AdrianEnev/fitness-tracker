...
interface ManualSyncModalProps {
    ...
    exercises: any;
    workoutTitle: string;
    isManualSyncModalVisible: boolean;
    setIsManualSyncModalVisible: (isVisible: boolean) => void;
    duration: any;
    internetConnected: boolean;
}

const ManualSyncModal: React.FC<ManualSyncModalProps> = ({ 
    isManualSyncModalVisible, setIsManualSyncModalVisible, navigation, exercises, workoutTitle, duration, internetConnected 
}) => { 
    
    //console.log('ManualSyncModal.tsx: ', duration);
    
    return (
        <Modal
            animationType="fade"
            transparent={true}
            visible={isManualSyncModalVisible}
            onRequestClose={() => {
                setIsManualSyncModalVisible(!isManualSyncModalVisible);
            }}
            >
                <View style={tw`flex-1 justify-center items-center mx-3`}>
                    <Pressable style={tw`bg-white w-full h-[24%] rounded-2xl pt-3 px-2`} onPress={Keyboard.dismiss}>
                        ...
                        <Text style={tw`text-lg text-center font-medium mt-1`}>Sync Info</Text>
                        ...
                        <View style={tw`flex items-center mt-2`}>
                            <Pressable style={tw`bg-[#e83d50] w-full h-10 rounded-xl flex items-center justify-center`} onPress={() => {
                                ...
                                <Text style={tw`text-white text-lg font-medium`}>Sync</Text>
                            </Pressable>
                        </View>

                        <View style={tw`flex items-center mt-2`}>
                            <Pressable style={tw`bg-gray-200 w-full h-10 rounded-xl flex items-center justify-center`} onPress={() => setIsManualSyncModalVisible(false)}>
