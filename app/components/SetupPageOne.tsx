    ...
    return (
        <View style={tw`flex flex-col mt-[15%] h-full`}>
            <View style={tw`mx-5`}>
                ...
                <Text style={tw`font-medium text-lg text-gray-500 mt-1 text-center`}>This will be used to calculate your daily BMR!</Text>
            </View>
            ...
            <View style={tw`flex-1 flex-col gap-y-5 mx-5 mt-[10%]`}>
                ...
                <Pressable style={tw`w-full h-[12%] rounded-2xl ${includeInBio === false ? 'bg-[#fd3e4b]' : 'bg-green-400'} flex flex-row gap-x-2 items-center pl-3`}
                    onPress={() => setIncludeInBio(!includeInBio)}
                >

                    <Ionicons name={includeInBio === false ? 'close' : 'checkmark-outline'} size={55} color='white'/>
