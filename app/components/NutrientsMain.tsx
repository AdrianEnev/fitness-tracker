...
const Nutrients = ({ currentNutrients, navigation, formattedDate, regularDate, goalNutrients}: RenderNutrientsProps) => {
    ...
    return (
        <Pressable style={tw`mt-3 mx-2 w-full h-66`} onPress={() => {
            ...
            <View style={tw`w-[98.5%] ${iphoneModel.includes('Ipad') ? "h-92" : "h-66"} shadow-lg pt-2 pl-3 bg-white rounded-lg`}>
                
                <View style={tw`flex flex-row justify-between`}>

                    <View style={tw`flex flex-col`}>
                        <Text style={tw`${iphoneModel.includes('Ipad') ? "text-6xl m-2" : "text-2xl"} font-medium`}>{t('calories')}</Text>
                        <Text style={tw`${iphoneModel.includes('Ipad') ? "text-2xl m-2" : "text-lg"} text-gray-600 mt-[-5px]`}>{regularDate}</Text>
                    </View>

                    <AnimatedCircularProgress
                        key={goalCalories}
                        style={tw`mr-3`}
                        size={iphoneModel.includes('Ipad') ? 230 : 160}
                        width={15}
                    ...
                    >
                        {() => (
                            <View style={tw`flex flex-row items-center justify-center`}>
                                ...
                                <Text style={tw`${currentCalories <= 9999 ? 'text-2xl' : 'text-xl'} font-medium ${iphoneModel.includes('Ipad') ? "text-5xl" : ""}`}>{currentCalories <= 50000 ? currentCalories : '50000'}/</Text>
                                <Text style={tw`text-base font-medium mt-2 ${iphoneModel.includes('Ipad') ? "text-2xl" : ""}`}>{goalCalories}</Text>
