...
const SetupPageThree = ({weight, weightType, setWeight, setWeightType, setHeightType, setHeight, height}: any) => {
    ...
    const ItemsKG = Array.from({length: 121}, (_, i) => i + 30);
    const ItemsLB = ItemsKG.map((item: any) => Math.round(item * 2.20462));
    ...
    return (
        <View style={tw`flex flex-col mt-[15%] h-full`}>
            <View style={tw`mx-5`}>
                <Text style={tw`font-medium text-2xl text-center`}>{t('setup-weight')}</Text>
                {currentLanguage === 'en' ? 
            ...
            <View style={tw`mx-3 mt-5 w-[94.5%] h-full`}>
                <View style={tw`w-full h-[18%] bg-gray-200 rounded-[47px]`}>
                    ...
                    <HorizontalPicker
                        ...
                        data={weightType === "KG" ? ItemsKG : ItemsLB}
                        ...
                        onChange={(index: any) => {
                            if (weightType === "KG") {
                                ...
                                setWeight(Math.round((index + 30) * 2.20462));
                            }
                        }}
                    />
